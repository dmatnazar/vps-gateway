...
function migrateToV13(db) {
    ...
    if (ver >= 13)
        return;
    try {
        const cols = db.prepare(`PRAGMA table_info(wallet_ledger)`).all().map((r) => r.name);
    ...
    db.prepare(`INSERT OR REPLACE INTO schema_version (version, applied_at) VALUES (13, datetime('now'))`).run();
