    ...
    async replaceEndpoints(tenantId, endpoints) {
      ...
      `);
            for (const ep of endpoints) {
                const epId = ep.id || node_crypto_1.default.randomUUID();
                stmt.run(epId, tenantId, tenant.slug, ep.name, ep.method, ep.pathTemplate, ep.sqlQuery, JSON.stringify(ep.paramsSchema || {}), ep.responseSchema ? JSON.stringify(ep.responseSchema) : null, ep.cacheTtlSec || 0, ep.authRequired ? 1 : 0, ep.dbKey || 'primary', ep.connectionId || '', ep.database || '', now, now);
            }
        });
        tx();
