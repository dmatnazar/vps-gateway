...
import { bindParams, extractParamValues, MissingParamError, expandMultiValueParams } from './paramBinder';
...
async function handleDynamic(
  ...
  if (agentTunnelManager.isAgentOnline(tenantSlug)) {
    try {
      ...
      const expanded = expandMultiValueParams(endpoint.sqlQuery || '', extractedParams as Record<string, unknown>);
      req.log.info(
        { tenantSlug, endpoint: endpoint.name, extractedParams, expandedParams: expanded.params },
        'dispatching-query-via-agent-tunnel'
      );
  ...
  try {
    ...
    const boundSql = bindParams(sqlRequest, endpoint, params, req, endpoint.sqlQuery);
