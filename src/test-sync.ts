...
async function runTests() {
  ...
  let foundTenant = await tenantRepository.findBySlug(testSlug);
  ...
  await tenantRepository.replaceConnections(tenant.id, [
    {
      dbKey: 'primary',
      label: 'Main DB',
  ...
  foundTenant = await tenantRepository.findBySlug(testSlug);
  ...
  console.log('\n--- Test 2: Endpoint (API) CRUD ---');
  await tenantRepository.replaceEndpoints(tenant.id, [
    {
      name: 'getUsers',
      method: 'GET',
      pathTemplate: '/users',
  ...
  const updatedStaff = db.prepare(`SELECT role FROM staff WHERE id = ?`).get(staffId) as any;
  ...
  const logs = getSyncLogs({ limit: 10 });
  if (logs.length === 0) {
