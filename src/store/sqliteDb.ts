...
function migrateToV9(db: Database.Database) {
  ...
  const tenants = db.prepare(`SELECT id, slug FROM tenants`).all() as { id: string; slug: string }[];
...
export function logSync(
  ...
  entityType: 'tenant' | 'staff' | 'endpoint' | 'connection' | 'registration' | 'notification' | 'device' | 'tariff' | 'wallet' | 'subscription',
